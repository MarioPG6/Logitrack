...
export default {
  ...
  methods: {
    async cargarDatos() {
      try {
        const encRes = await axios.get("http://localhost:8080/encomiendas");
        const trabRes = await axios.get(
          "http://localhost:8080/admin/users/workers"
        );
    ...
    async asignar() {
      try {
        await axios.post("http://localhost:8080/asignaciones/asignar", null, {
          params: {
